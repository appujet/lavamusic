...
function neb(embed: EmbedBuilder, player: Dispatcher, client: Lavamusic): EmbedBuilder {
    ...
    let iconUrl = client.config.icons[player.current.info.sourceName] || client.user.displayAvatarURL({ extension: 'png' });
    let icon = player.current.info.artworkUrl || client.config.links.img;
