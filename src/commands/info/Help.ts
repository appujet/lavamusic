...
export default class Help extends Command {
    ...
    public async run(client: Lavamusic, ctx: Context, args: string[]): Promise<void> {
        ...
        const categories = commands.map((cmd) => cmd.category).filter((value, index, self) => self.indexOf(value) === index);
        ...
        if (!args[0]) {
            ...
            const helpEmbed = embed.setColor(this.client.color.main)
                .setTitle("Help Menu")
                .setDescription(`Hey there! I'm ${this.client.user.username}, a music bot made with [Lavamusic](https://github.com/brblacky/lavamusic) and Discord.js. You can use \`${prefix}help <command>\` to get more info on a command.`)
                .setFooter({ text: `Use ${prefix}help <command> for more info on a command` });
            fildes.forEach((field) => helpEmbed.addFields(field));
            ctx.sendMessage({ embeds: [helpEmbed] });
        } else {
            const command = this.client.commands.get(args[0].toLowerCase());
            if (!command) return ctx.sendMessage({ embeds: [client.embed().setColor(client.color.red).setDescription(`Command \`${args[0]}\` not found`)] });
            const embed = this.client.embed();
            const helpEmbed = embed.setColor(this.client.color.main)
