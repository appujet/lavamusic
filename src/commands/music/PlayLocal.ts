...
export default class PlayLocal extends Command {
    ...
    public async run(client: Lavamusic, ctx: Context): Promise<any> {
        ...
        const response = await player.search({
            ...
            source: "local"
        }, ctx.author).catch(() => null) as SearchResult | null;
        ...
        const queueWasEmpty = !player.queue.tracks.length && !player.queue.current;
        ...
        await ctx.editMessage({
            ...
            embeds: [this.client.embed()
                ...
                .setDescription(
                    ctx.locale(
                        queueWasEmpty
                            ? 'cmd.playlocal.now_playing'
                            : 'cmd.playlocal.added_to_queue',
                        { title: attachment.name, url: attachment.url }
                    )
                )]
        });
