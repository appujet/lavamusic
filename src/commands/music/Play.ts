    ...
    public async run(client: Lavamusic, ctx: Context, args: string[]): Promise<void> {
        ...
        switch (res.loadType) {
            ...
            case 'TRACK_LOADED':
                ...
                if (player.queue.length > client.config.maxQueueSize) return ctx.sendMessage({ embeds: [embed.setColor(this.client.color.red).setDescription(`The queue is too long. The maximum length is ${client.config.maxQueueSize} songs.`)] });
            ...
            case 'PLAYLIST_LOADED':
                if(res.length > client.config.maxPlaylistSize) return ctx.sendMessage({ embeds: [embed.setColor(this.client.color.red).setDescription(`The playlist is too long. The maximum length is ${client.config.maxPlaylistSize} songs.`)] });
