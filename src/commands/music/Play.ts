        ...
        if (res.data.tracks.length > client.config.maxPlaylistSize)
          return await ctx.editMessage({
            ...
            embeds: [
              embed
                .setColor(this.client.color.red)
                .setDescription(ctx.locale("cmd.play.errors.playlist_too_long", { maxPlaylistSize: client.config.maxPlaylistSize })),
            ],
          });
      ...
      case LoadType.SEARCH: {
        ...
        await ctx.editMessage({
          ...
          embeds: [
            embed
              .setColor(this.client.color.main)
              .setDescription(ctx.locale("cmd.play.added_to_queue", { title: res.data[0].info.title, uri: res.data[0].info.uri })),
          ],
        });
        break;
      }
