...
module.exports = {
  ...
  execute: async (message, args, client, prefix) => {
    ...
    collector.on("end", async () => {
      if (!m) return;
      await m.edit({
        embeds: [embed1.setDescription(`Time is up! Run ${prefix}filters`)],
      });
    });
    collector.on("collect", async (b) => {
      ...
      if (b.customId === "clear_but") {
        ...
        return await b.editReply({
          embeds: [
            embed1.setDescription(`${emojiequalizer} Night Core mode is ON`),
          ],
        });
        ...
        if (m) await m.edit({ embeds: [embed], components: [row, row2] });
        return await b.editReply({
          embeds: [embed1.setDescription(`${emojiequalizer} Pitch mode is ON`)],
        });
