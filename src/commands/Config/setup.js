      ...
      if (args.length) {
        if (!data)
          return await message.reply({
            content: `This server doesn't have any song request channel setup to use this sub command.`,
          });
        if (["clear", "delete", "reset"].includes(args[0])) {
          console.log(data);
          await data.delete();
          return await message.reply({
            content: `Successfully deleted all the setup data.`,
          });
        } else
          return await message.reply({
        ...
        if (data)
          return await message.reply({
            content: `Music setup is already finished in this server.`,
          });

        const parentChannel = await message.guild.channels.create(
          `${client.user.username} Music Zone`,
          {
            type: "GUILD_CATEGORY",
        ...
        const textChannel = await message.guild.channels.create(
          `${client.user.username}-song-requests`,
          {
            type: "GUILD_TEXT",
        ...
        const title =
          player && player.queue && player.queue.current
            ? `Now playing`
        ...
        let pausebut = new MessageButton()
          .setCustomId(`pause_but_${message.guildId}`)
          .setEmoji("⏯️")
          .setStyle("SECONDARY")
          .setDisabled(disabled);

        let lowvolumebut = new MessageButton()
          .setCustomId(`lowvolume_but_${message.guildId}`)
          .setEmoji("🔉")
          .setStyle("SECONDARY")
          .setDisabled(disabled);

        let highvolumebut = new MessageButton()
