...
export default class TrackStart extends Event {
    ...
    public async run(player: Player, track: Song, dispatcher: Dispatcher) {
        ...
        const embed = this.client.embed()
            ...
            .addFields(
                { name: "Duration", value: track.info.isStream ? "LIVE" : this.client.utils.formatTime(track.info.length), inline: true },
        ...
        collector.on('collect', async (interaction) => {
            switch (interaction.customId) {
                ...
                case 'resume':
                    dispatcher.pause();
                    if (message) await message.edit({ embeds: [embed.setFooter({ text: `${player.paused ? 'Paused' : 'Resumed'} by ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({}) })], components: [button] });
                    break;
                case 'stop':
                    dispatcher.stop();
                    if (message) await message.edit({ embeds: [embed.setFooter({ text: `Stopped by ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({}) })], components: [] });;
                    break;
                case 'skip':
                    ...
                    dispatcher.skip();
                    if (message) await message.edit({ embeds: [embed.setFooter({ text: `Skipped by ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({}) })], components: [button] });
                    break;
                case 'loop':
                    switch (dispatcher.loop) {
                        case 'off':
                            dispatcher.loop = 'repeat';
                            if (message) await message.edit({ embeds: [embed.setFooter({ text: `Looping by ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({}) })], components: [button] });
                            break;
                        case 'repeat':
                            dispatcher.loop = 'queue';
                            if (message) await message.edit({ embeds: [embed.setFooter({ text: `Looping Queue by ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({}) })], components: [button] });
                            break;
                        case 'queue':
                            dispatcher.loop = 'off';
                            if (message) await message.edit({ embeds: [embed.setFooter({ text: `Looping Off by ${interaction.user.tag}`, iconURL: interaction.user.avatarURL({}) })], components: [button] });
                            break;
                    }
                    break;
            }
