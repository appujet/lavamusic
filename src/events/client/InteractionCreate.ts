    ...
    if (interaction instanceof CommandInteraction && interaction.isCommand()) {
      ...
      if (command.player) {
        if (command.player.voice) {
          ...
          if (clientMember.voice.channel && clientMember.voice.channelId !== (interaction.member as GuildMember).voice.channelId) {
            return await interaction.reply({
        ...
        if (command.player.dj) {
          ...
          if (dj?.mode) {
            ...
            const hasDJRole = (interaction.member as GuildMember).roles.cache.some((role) => djRole.map((r) => r.roleId).includes(role.id));
