...
export default class ShoukakuClient extends Shoukaku {
    ...
    constructor(client: Lavamusic, nodes: NodeOption[]) {
        super(new Connectors.DiscordJS(client), nodes, {
            ...
            nodeResolver: (nodes) => ShoukakuClient.getBestNode(nodes),
        });
    ...
    private static getBestNode(nodes: Map<string, any>) {
        const bestNode = [...nodes.values()]
            ...
            .sort((a, b) => {
                ...
                if (a.stats.playingPlayers !== b.stats.playingPlayers) return a.stats.playingPlayers - b.stats.playingPlayers;
        ...
        for (const [guildId, player] of this.client.queue.entries()) {
            if (player.node.name === nodeName) {
                try {
                    ...
                    logger.info(`Disconnected player in guild ${guildId} due to node failure`);
                } catch (error) {
                    logger.error(`Failed to disconnect player in guild ${guildId}: ${error.message}`);
                }
            }
        }
    }
                ...
                if (existingPlayer) {
                    ...
                    try {
                        ...
                        if (!error.message.includes("existing connection")) {
                            logger.error(`Failed to reconnect 24/7 player in guild ${guild.id}: ${error.message}`);
                        }
                    }
                }
            }
