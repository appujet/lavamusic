...
export class BotClient extends Client {
  ...
  async loadEvents() {
    ...
    eventFiles.forEach(async (file) => {
      const events = readdirSync(`./src/events/${file}`).filter(
        (c) => c.split(".").pop() === "js"
      );
      events.forEach(async (event) => {
        ...
        const eventName =
          event.split(".")[0].charAt(0).toLowerCase() +
          event.split(".")[0].slice(1);
  ...
  async loadCommands() {
    ...
    commandFiles.forEach((file) => {
      const commands = readdirSync(`./src/commands/${file}`).filter((file) =>
        file.endsWith(".js")
      );
        ...
        (async () => {
          try {
            const rest = new REST({ version: "10" }).setToken(
              this ? this.config.token : config.token
            );
            await rest.put(
              Routes.applicationCommands(
                this ? this.config.clientId : config.clientId
              ),
              { body: cmdData }
            );
          } catch (e) {
